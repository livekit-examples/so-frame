"""Debug harness for the sim2real mapping -- verify the wiring WITHOUT the policy.

`policy/run.py` chains three transforms; a mistake in any one silently feeds the
policy an out-of-distribution obs or drives the arm wrong:

  1. camera mapping   raw wide-FOV frame -> 128x128 sim view (per camera)
  2. RGB stacking     wrist(0:3) | overhead(3:6) -> 6-channel obs, squinted to 32
  3. joint bridge     real deg/0-100 <-> sim rad/m, and sim target -> real target

Exercises all three with NO checkpoint and NO motion (except --control). Modes:

    # 1. camera mapping on a saved frame (needs the frame + mapping JSON)
    python debug_policy.py --frame snap_overhead.png --camera overhead --out /tmp
    #    writes <out>/debug_overhead.png = raw | rectified-128 | squint-32.

    # 2. joint bridge self-test (needs nothing; default with no args)
    python debug_policy.py --bridge
    #    round-trips every joint and shows the rail's calibrated 0-100 mapping.

    # 3. live pipeline, READ-ONLY (never claims control, never sends an action)
    python debug_policy.py --live --out /tmp --every 1.0
    #    every --every s: dumps live_*.png + prints the state bridge round-trip.

    # 4. MANUAL CONTROL -- THIS MOVES THE ROBOT. Drives joints by hand in SIM
    #    units to verify the bridge/mapping (esp. the rail). Ramps at trained speed.
    python debug_policy.py --control
    #    REPL: `far`/`near` (rail endpoints), `rest`, `<joint> <val>`, `n <joint> <d>`, `p`, `q`.

Reuses the same bridge / camera_mapping / CAMERA_STACK as run.py, so what it
shows is what the policy gets.
"""
...
# Same wrist-first, overhead-second order run.py stacks into the sim RGB obs.
CAMERA_STACK = (
...
# ---------------------------------------------------------------------------
# shared: rectify a frame exactly as run.py does, and squint like the policy
# ---------------------------------------------------------------------------
...
def _mapping_for(track: str) -> dict | None:
    ...
    print(f"[debug] {track}: NO mapping ({fname} missing) -- using a plain 128px "
          "resize (OUT OF DISTRIBUTION; this is the same fallback run.py logs)")
    return None


def rectify(frame_rgb: np.ndarray, mapping: dict | None) -> np.ndarray:
    """Raw frame -> 128x128 sim view. Mirrors run.py.rectify."""
...
def squint(view128: np.ndarray) -> np.ndarray:
    """128 -> 32 area-downsample, the policy's input resolution (matches
    agent.SquintPolicy._squint)."""
    return cv2.resize(view128, (SQUINT_SIZE, SQUINT_SIZE), interpolation=cv2.INTER_AREA)


def _labeled(img_rgb: np.ndarray, label: str, size: int = 256) -> np.ndarray:
    """Upscale (nearest, so pixels stay honest) to `size` and caption it, BGR out."""
...
# ---------------------------------------------------------------------------
# mode 1: camera mapping on a saved frame
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# mode 2: joint bridge self-test
# ---------------------------------------------------------------------------

def run_bridge() -> int:
    """Round-trip every joint real<->sim<->real at its limits + midpoint, assert
    consistency, and print the rail's calibrated [0,1] far/close mapping."""
...
# ---------------------------------------------------------------------------
# mode 3b: pull one RAW frame per camera through Portal (for calibration)
# ---------------------------------------------------------------------------

async def run_snapshot(out_dir: pathlib.Path) -> int:
    """Join the room read-only, grab one RAW frame per camera, save, and exit.
    The saved frames feed examples/move_sim_camera.py to calibrate the mappings."""
    from livekit.portal import (
        Observation, Operator, OperatorConfig, frame_bytes_to_numpy_rgb,
    )
    from common import env, load_env, mint_token, pace
...
# ---------------------------------------------------------------------------
# mode 3: live read-only pipeline against the robot
# ---------------------------------------------------------------------------

async def run_live(out_dir: pathlib.Path, every: float) -> int:
    """Join the room read-only; periodically dump the live rectified view and the
    state bridge round-trip. Never claims control, never sends an action."""
    ...
    try:
        async for _ in pace(fps):
            ...
            # state bridge round-trip on the LIVE pose
            ...
            # rectified live view per camera
            for track, _ in CAMERA_STACK:
...
# ---------------------------------------------------------------------------
# mode 4: manual control -- drive joints by hand to verify the bridge on hardware
# ---------------------------------------------------------------------------

# Sim rest pose, JOINT_KEYS order (== SO101OnFrame.keyframes["rest"].qpos).
_SIM_REST = {
    "dof_slider.pos": 0.0, "shoulder_pan.pos": 0.0, "shoulder_lift.pos": -0.5,
    "elbow_flex.pos": 0.8, "wrist_flex.pos": 0.6, "wrist_roll.pos": 0.0,
    "gripper.pos": 1.2,
}
...
async def run_control(assume_yes: bool) -> int:
    """Claim the robot and drive joint targets by hand, ramping to each goal at the
    trained per-tick DELTA_LIMIT speed. Verifies the bridge/mapping on real hardware."""
    ...
    # Background stdin reader -> queue, so the send loop never blocks on input.
    ...
    try:
        async for _ in pace(fps):
            ...
            if stop["quit"]:
                break
            # Ramp target toward goal at the trained per-tick delta (safe speed).
            for k in bridge.JOINT_KEYS:
                d = goal[k] - target[k]
...
# ---------------------------------------------------------------------------
# mode 4b: manual control via an OpenCV UI (trackbars + live camera), no typing
# ---------------------------------------------------------------------------
...
async def run_control_ui(assume_yes: bool) -> int:
    """Manual control via an OpenCV window: one trackbar per joint (sim units) + the
    live rectified camera view, so you drive the robot and watch what the policy sees
    without typing. Targets ramp to each trackbar goal at the trained per-tick speed.
    ...
    mappings = {t: _mapping_for(t) for t, _ in CAMERA_STACK}
    # Speed check: the sim commands each joint at DELTA_LIMIT per 10 Hz step; in wire
    # units that is DELTA_LIMIT * |SCALE| * fps. Measure the real robot's achieved max
    # speed from the observed state -- if it's well below the sim number, the hardware
    # can't keep up and arm_speed_scale / the delta limits should come down to match.
    ...
    try:
        async for _ in pace(fps):
            ...
            # measure real achieved joint speed (wire units / s) from observed state
            if obs is not None:
                now = time.perf_counter()
            ...
            if key == ord("c"):
                for k in max_real_speed:
                    max_real_speed[k] = 0.0   # reset the measured max
    except KeyboardInterrupt:
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Debug the sim2real camera mapping + joint bridge (no policy, no motion).",
    ...
    # default / --bridge: the zero-dependency self-test
    return run_bridge()
