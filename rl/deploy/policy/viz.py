...
policy runs rather than after. Below them, one bar per joint: the action the last decision produced
...
class _Panel(QtWidgets.QLabel):
    ...
    def show_rgb(self, rgb):
        ...
        self.setPixmap(QtGui.QPixmap.fromImage(img).scaled(
            self.size_px - 2, self.size_px - 2, QtCore.Qt.KeepAspectRatio,
...
class _Actions(QtWidgets.QWidget):
    ...
    ROW, PAD, LABEL = 22, 6, 74

    def __init__(self, names):
        super().__init__()
        self.names = list(names)
        self.rows: list = []
        self.setFixedHeight(self.ROW * len(self.names) + 2 * self.PAD)
    ...
    def paintEvent(self, _ev):          # noqa: N802 - Qt naming
        ...
        for i, name in enumerate(self.names):
            ...
            if act is not None:
                ...
                p.drawText(int(x0 + span + 6), y + 15, f"{a:+.2f}")
...
class Window(QtWidgets.QWidget):
    def __init__(self, cameras, joint_names, size=300):
        super().__init__()
        self.setWindowTitle("so-frame policy")
        self.setStyleSheet(STYLE)
        self.requests: list[str] = []      # single-char keys, drained by the caller each tick

        self.panels = {c: _Panel(c, size) for c in cameras}
        row = QtWidgets.QHBoxLayout()
        for c, panel in self.panels.items():
            col = QtWidgets.QVBoxLayout()
            lab = QtWidgets.QLabel(c)
        ...
        hint = QtWidgets.QLabel("action out of centre; the tick is how far the arm lags its target")
