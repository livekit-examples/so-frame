...
then fit the real cameras against the PNGs with sim2real/utils/calibrate_camera.py (needs only
OpenCV, so it also runs on the robot host).

    uv run python examples/dump_reference_views.py
    uv run python examples/dump_reference_views.py --out ../../sim2real/utils/reference_views
...
parser.add_argument("--out", default="reference_views",
                    help="directory for the PNGs (default: rl/maniskill/reference_views)")
...
print()
print("Now fit each real camera against these, from sim2real/:")
