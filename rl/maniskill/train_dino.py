"""Squint SAC, DINOv2 variant: frozen ViT features instead of a from-scratch CNN.

Identical to `train_squint.py` except the vision encoder is a FROZEN DINOv2 ViT-S/14
(registers) backbone + a small trainable conv head (see `DinoEncoder`), swapped in for
`CNNEncoder`. Everything else (SAC, C51 critic, DR, reward, loop) is unchanged.
...
replay buffer (run the ViT once per env-step at rollout, train only the head on cached
features); that needs a buffer restructure (and a smaller/bf16 buffer, the grid is large)
and is left as a follow-up. First run downloads DINOv2 weights via torch.hub (needs
internet + ~/.cache/torch/hub).

Usage (from rl/maniskill/, after `uv sync`):
    uv run python train_dino.py --env_id=SOFramePickPlaceBin-v1
...
class DinoEncoder(nn.Module):
    """Frozen DINOv2 ViT-S/14 (registers) backbone + a small trainable conv head.

    Drop-in for CNNEncoder: same interface (`__init__(n_obs, device)`, `.repr_dim`,
    `forward(rgb_uint8[B,H,W,C]) -> [B, repr_dim]`). The stacked cameras (C = 3*num_cams)
    are split, each run through DINOv2 to a patch-token grid, the grids concatenated on
    the channel axis, and a 2-layer conv head maps them to 1024 dims (== CNNEncoder.repr_dim,
    so Actor/Critic are unchanged).

    The frozen backbone is kept OUT of `parameters()`/`state_dict()` (stored via
    object.__setattr__, like Critic._q_meta), so the optimizer trains only the head,
    checkpoints don't carry ~90 MB of ViT weights, and the from_module weight-sharing
    across encoder copies shares just the head.
    """
    ...
    def __init__(self, n_obs, device=None):
        ...
        in_ch = self.num_cams * self.EMBED
        self.head = nn.Sequential(
            nn.Conv2d(in_ch, 256, 3, stride=1, padding=1, device=device), nn.ReLU(),
            nn.Conv2d(256, 128, 3, stride=2, device=device), nn.ReLU(),
            nn.Flatten(),
        )
        with torch.no_grad():
            n_flat = self.head(torch.zeros(1, in_ch, self.grid, self.grid, device=device)).shape[-1]
        self.fc = nn.Linear(n_flat, self.repr_dim, device=device)
        self.apply(weight_init)

    def _grid(self, img3):
        # img3: [B, 3, dino_res, dino_res] normalized -> [B, EMBED, grid, grid]
        tok = self._dino.forward_features(img3)["x_norm_patchtokens"]
        b = tok.shape[0]
        return tok.transpose(1, 2).reshape(b, self.EMBED, self.grid, self.grid)

    def forward(self, obs):
        # obs: [B, H, W, C] uint8, C = 3 * num_cams
        x = obs.permute(0, 3, 1, 2).float() / 255.0
        with torch.no_grad():  # frozen backbone: features detach from the head's graph
            grids = []
            for c in range(self.num_cams):
                ...
                grids.append(self._grid(cam))
            feat = torch.cat(grids, dim=1)
        return self.fc(self.head(feat))  # trainable head
