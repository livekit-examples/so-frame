...
One window. **REAL | SIM | OVERLAY** for the camera being fitted, a small strip for the other one,
and a rail of numbers on the right that drive everything.

The **arm** fields drive the robot, one per joint, ranged to the sim joint limits so you cannot
command outside them. Targets ramp at the trained per-tick speed rather than snapping.

The **camera mapping** fields fit the mapping: `rot90` / `angle` / `k1` / `k2` / `focal` / `zoom` /
`offset x` / `offset y`. Type a value or use the arrows. They reseed from the saved mapping when you
switch cameras, so you are always nudging the fit that currently ships.
...
The slider under the panels sets the overlay mix, real at one end and sim at the other. Buttons:
...
- drive one joint to each end. The field limits are the URDF limits, so the real joint should hit
...
The rail also shows achieved-vs-commanded joint speed. If the measured real maximum stays well under the
